...
def create_lstm(input_shape):
    ...
    x1 = tf.keras.layers.LSTM(50, activation='relu',return_sequences = True,name='lstm')(x)
    x1 = tf.keras.layers.BatchNormalization()(x1)
    x6 = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(1000, activation='relu',kernel_regularizer = tf.keras.regularizers.l2(0.01)))(x1)
    x6 = tf.keras.layers.BatchNormalization()(x6)
    x6 = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(200, activation='relu',kernel_regularizer = tf.keras.regularizers.l2(0.01)))(x6)
    x6 = tf.keras.layers.BatchNormalization()(x6)
    x6 = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(200, activation='relu',kernel_regularizer = tf.keras.regularizers.l2(0.01)))(x6)
    x6 = tf.keras.layers.BatchNormalization()(x6)
...
def data_split(idx, n_folds, groups):
    ...
    gkf = GroupShuffleSplit(n_splits=n_folds ,test_size=0.30, random_state=10)
    ...
    for train_val_split, test_split in gkf.split(idx, groups=groups):
        ...
        gss = GroupShuffleSplit(n_splits=1,test_size=0.01,random_state=10) #here test_size is actually "validation_size"
...
dense = 0 #train or evaluate dense model
...
result = np.zeros((len(max_depth),len(min_samples_split),len(min_samples_leaf)))

# =============================================================================
# for a in range(len(max_depth)):
#     for b in range(len(min_samples_split)):
...
for i in range(n_folds):
    ...
    clf.fit(x_trainDense, y_trainDense)
    y_pred_dense = clf.predict_proba(x_testDense)
    y_pred_dense = np.reshape(y_pred_dense[:,1], (test_shape[0],test_shape[1],1))
    Mask = x_test!=0
...
# =============================================================================
#             result[a,b,c]=Step_Err_Dense.mean(0)[-1]
#             print('\r'+' Max Depth:' + str(a) + ' Split:' + str(b)+ ' Leaf:' + str(c),end=' \r ') 
# =============================================================================
...
# path = clf.cost_complexity_pruning_path(x_trainDense, y_trainDense)
...
# train_scores = [c.score(x_trainDense, y_trainDense) for c in clf]
# test_scores = [c.score(x_testDense, y_testDense) for c in clf]
# ax.set_xlabel("alpha")
...
tree.plot_tree(clf,
               max_depth=None, 
               feature_names=['F','dF','ddF','dddF','s','ds','dds','ddds'], 
               class_names=None, 
               label='root', 
               filled=True, 
               impurity=True,
