...
def data_split(idx,n_splits,groups):
    ...
    gkf = GroupShuffleSplit(n_splits=n_splits,test_size=0.2,random_state=10)
    ...
    for i in range(id_from,id_to):
        for j in range(1,4):
            ...
            if F.size > 0 and Fo.size > 0 and to.size > 0 and to > 0.1:
                ...
                to = round(to[0],2)
                # find change point index in array
                idxo = np.searchsorted(t,to,'left')
                # Force signal without preprocessing
                F_raw.append(F)
                Fos.append(Fo)
                # change point defined as 1, others as 0
                y = np.zeros_like(F)
                y[idxo] = 1
            else:
                 continue
