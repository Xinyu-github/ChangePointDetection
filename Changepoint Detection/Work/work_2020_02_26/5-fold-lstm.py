# -*- coding: utf-8 -*-
"""
Created on Wed Feb 26 10:25:16 2020
...
# LSTM NN
    ss = ShuffleSplit(n_splits=5,test_size=test_vol,random_state=10)
    for trainidx, testidx in ss.split(trial):
        test = uniidx[testidx]
        trial_test = label[testidx,0]
        trial_train = label[trainidx,0]
        
        test = test[np.isin(trial_test, trial_train)]    
        ...
        y_pred_lstm = model_lstm.predict(recurr_xtest)
        Err_lstm, lstm_err = evaluate_plot(y_pred_lstm,to=idxo_masked[test])
