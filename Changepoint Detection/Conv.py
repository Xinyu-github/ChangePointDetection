...
def create_lstm(input_shape):
    ...
    y_ = tf.keras.layers.BatchNormalization()(x1)
...
def create_dense(input_shape):
    inputs = tf.keras.Input(shape = [622,20])
    x6 = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(10, activation='relu'))(inputs)
    x6 = tf.keras.layers.BatchNormalization()(x6)
    x6 = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(10, activation='relu'))(x6)
    x6 = tf.keras.layers.BatchNormalization()(x6)
    x6 = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(5, activation='relu'))(x6)
...
loadmodel = 1 #load trained neural network and data splits
...
for i in range(n_folds):
    ...
    model_dense = create_dense(x_train.shape[1:])
